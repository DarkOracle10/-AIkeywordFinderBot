...
class TelegramSearchApp:
    ...
    def _create_widgets(self):
        ...
        self.status_label = ttk.Label(
            header_frame, text="❌ Not logged in", font=("Segoe UI", 10)
        )
    ...
    def _create_login_tab(self):
        ...
        phone_frame = ttk.LabelFrame(
            self.login_frame, text="Step 1: Enter Phone Number", padding="10"
        )
        ...
        code_frame = ttk.LabelFrame(
            self.login_frame, text="Step 2: Enter Verification Code", padding="10"
        )
        ...
        self.verify_code_btn = ttk.Button(
            code_frame,
            text="✓ Verify Code",
        ...
        # 2FA frame
        tfa_frame = ttk.LabelFrame(
            self.login_frame,
            text="Step 3: Two-Factor Authentication (if enabled)",
            padding="10",
        )
        ...
        self.verify_tfa_btn = ttk.Button(
            tfa_frame,
            text="🔐 Verify 2FA",
            command=self._on_verify_2fa,
            style="Primary.TButton",
        )
    ...
    def _create_search_tab(self):
        ...
        # Chats frame
        chats_frame = ttk.LabelFrame(
            self.search_frame, text="Chats & Folders to Search", padding="10"
        )
        chats_frame.pack(fill=tk.X, pady=(0, 10))

        ttk.Label(
            chats_frame,
            text="Enter 'all', chat names, or folders with <folder_name> syntax:",
        ).pack(anchor=tk.W)
        ttk.Label(
            chats_frame,
            text="Examples: all | <Work>, <Friends> | chat1, chat2, <MyFolder>",
        ...
        self.folders_btn = ttk.Button(
            chats_frame, text="📁 Show My Folders", command=self._on_show_folders
        )
        self.folders_btn.pack(pady=5)

        # Exclude frame
        exclude_frame = ttk.LabelFrame(
            self.search_frame, text="Exclude (Optional)", padding="10"
        )
        exclude_frame.pack(fill=tk.X, pady=(0, 10))

        ttk.Label(
            exclude_frame,
            text="Exclude chats/folders using the same syntax (<folder_name>, chat):",
        ).pack(anchor=tk.W)
        ttk.Label(
            exclude_frame,
            text="Examples: <Muted>, Ads, <SpamFolder> | leave empty for none",
            font=("Segoe UI", 9, "italic"),
            foreground="#666666",
        ).pack(anchor=tk.W)
        ...
        keywords_frame = ttk.LabelFrame(
            self.search_frame, text="Keywords", padding="10"
        )
        ...
        self.progress_label = ttk.Label(
            self.search_frame, textvariable=self.progress_var, font=("Segoe UI", 10)
        )
    ...
    def _create_results_tab(self):
        ...
        self.results_count = ttk.Label(
            self.results_frame, text="No search performed yet", font=("Segoe UI", 10)
        )
        self.results_count.pack(anchor=tk.W, pady=(0, 5))

        # Results text area
        self.results_text = scrolledtext.ScrolledText(
            self.results_frame, wrap=tk.WORD, font=("Consolas", 10), height=25
        )
        self.results_text.pack(fill=tk.BOTH, expand=True)

        # Configure tags for styling
        self.results_text.tag_configure(
            "header", font=("Segoe UI", 10, "bold"), foreground="#0066cc"
        )
        ...
        self.results_text.tag_bind(
            "link", "<Leave>", lambda e: self.results_text.config(cursor="")
        )
    ...
    async def _check_session(self):
        ...
        try:
            ...
            self.root.after(
                0, lambda: self._update_login_state(is_logged_in, user_name)
            )
    ...
    def _on_show_folders(self):
        ...
        async def get_folders():
            try:
                ...
                if not client:
                    self.root.after(
                        0, lambda: messagebox.showerror("Error", "Session expired")
                    )
                    return
    ...
    def _on_search(self):
        ...
        self.progress_var.set(
            f"🔄 Searching {' + '.join(search_desc) if search_desc else 'all chats'}..."
        )
    ...
    def _display_results(self, hits):
        ...
        for i, (dialog, msg) in enumerate(hits[:50]):
            ...
            if msg_link:
                link_tag = f"link_{i}"
                self.results_text.tag_configure(
                    link_tag, foreground="#0066cc", underline=True
                )
                self.results_text.insert(tk.END, "🔗 Open Message", link_tag)
                self.results_text.tag_bind(
                    link_tag, "<Button-1>", lambda e, url=msg_link: webbrowser.open(url)
                )
                self.results_text.tag_bind(
                    link_tag,
                    "<Enter>",
                ...
                self.results_text.tag_bind(
                    link_tag, "<Leave>", lambda e: self.results_text.config(cursor="")
                )
